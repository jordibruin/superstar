...
extension TerritoryCode {
    ...
    var flag: String {
        ...
        for code in Locale.isoRegionCodes {
            if let threeCode = CountryUtilities.getAlphaThreeCode(byAlpha2Code: code) {
                if threeCode == self.rawValue {
    ...
    var name: String? {
        ...
        for code in Locale.isoRegionCodes {
            if let threeCode = CountryUtilities.getAlphaThreeCode(byAlpha2Code: code) {
                if threeCode == self.rawValue {
                    if let name = NSLocale.current.localizedString(forRegionCode: code) {
                        corName = name
                    }
                }
            }
